...
HF_ENDPOINT=https://hf-mirror.com                 # HF 国内镜像加速
...
```bash
# 1)（如需）先在本机预处理数据，并设好数据目录环境变量（GRPO 用 ResponseDataset 的实验需要）
uv run lab prepare gsm8k
export GSM8K_DATA_DIR="$(pwd)/datasets/gsm8k"
...
- ✅ 上传：仓库代码（`common/`、`configs/`、`experiments/` 等）。自定义环境/奖励靠这个在所有节点被 `import`。
- ❌ 不上传：`datasets/**`、`**/outputs/**`、`.git/**`、`__pycache__`、`cluster/submit.env`、`cluster/secrets.env`、`*.key`（见 `submit_job.sh` 的 `excludes`）。
- 🔑 环境变量：`NEMO_RL_DIR` / `CLUSTER_PROFILE` 必传；`SWANLAB_API_KEY` / `HF_TOKEN` / `HF_ENDPOINT` / `HF_HUB_ENABLE_HF_TRANSFER` / `HF_HOME` 填了才转发（经 Ray `runtime_env`，不落盘）。

> 注意：数据集**不随作业上传**（太大且 `datasets/**` 被排除）。要么数据已在集群共享盘上，
> 要么模型/数据从 HF（配 `HF_ENDPOINT` 镜像）下载。`GSM8K_DATA_DIR` 等路径要指到**集群上**能访问的位置。
...
| 模型/数据下载慢或失败 | 配 `HF_ENDPOINT=https://hf-mirror.com`；gated 模型要 `HF_TOKEN`。 |
| `ray job submit` 版本协议报错 | Mac 的 `ray` 版本与集群差太多。对齐到相近版本。 |
