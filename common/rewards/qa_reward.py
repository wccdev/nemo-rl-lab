...
判分规则（模型答案须写进 \boxed{...}，否则 FORMAT_PENALTY）：
    ...
    multiple    : 见 MULTI_MODE
                  - "exact"          字母集合完全相等才 1.0（最稳，防刷）
                  - "partial_penalty"(默认) (选对−选错)/应选数，截断[0,1]，全选会被惩罚
                  - "f1"             预测集合与正确集合的 F1
...
FORMAT_PENALTY = -0.5
MULTI_MODE = "partial_penalty"  # "exact" | "partial_penalty" | "f1"
...
def _grade_multiple(pred: set[str], gold: set[str]) -> float:
    ...
    wrong = len(pred - gold)
    return max(0.0, (correct - wrong) / len(gold))
...
if __name__ == "__main__":
    ...
    cases = [
        ...
        ("[multiple] A,C,D", r"\boxed{A,B,C,D}", 2 / 3),   # 全选: (3-1)/3
