#!/usr/bin/env bash
# 实验启动脚本（NeMo-RL 0.6.0）。多轮 Agent·滑块拼图，入口是 NeMo-RL 自带 examples/run_grpo_sliding_puzzle.py。
...
# 训练入口：滑块拼图用 NeMo-RL 自带的多轮示例脚本（自带 env + 数据生成，无需本目录写 run.py）。
ENTRY="${ENTRY:-examples/run_grpo_sliding_puzzle.py}"
